...
func (sh *scheduler) trySchedMine() {
	...
	//if allowDelay >= 999 {
	//	log.Debugf("trySchedMine SCHED skipping...")
	//	return
	//}
...
func (sh *scheduler) assignWorkerNoLocker(taskDone chan struct{}, wid WorkerID, w *workerHandle, req *workerRequest) error {
	...
	go func() {
		...
		err := req.prepare(req.ctx, sh.workTracker.worker(wid, w.info, w.workerRpc))
		//sh.workersLk.Lock()
	...
	*/
	...
	for sqi := 0; sqi < queueLen; sqi++ {
		...
		for _, wnd := range acceptableWindows[task.indexHeap] {
			...
			windows[wnd].allocated.add(sh.allowMyScheduler, task.taskType, info.Resources, needRes)
