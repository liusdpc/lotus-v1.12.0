...
func (a *activeResources) withResources(allowMyScheduler bool, taskType sealtasks.TaskType, id WorkerID, wr storiface.WorkerInfo, r Resources, locker sync.Locker, cb func() error) error {
	...
	a.add(allowMyScheduler, taskType, wr.Resources, r)

	err := cb()

	a.free(allowMyScheduler, taskType, wr.Resources, r)
	if a.cond != nil {
		a.cond.Broadcast()
	}
...
func (a *activeResources) canHandleRequestMine(index stores.SectorIndex, sector abi.SectorID, taskType sealtasks.TaskType, worker *workerHandle, wid2 WorkerID, caller string) bool {
	...
	rpcCtx, cancel := context.WithTimeout(context.TODO(), WorkerRpcTimeout)
	info, err := worker.workerRpc.Info(rpcCtx)
	defer cancel()
	if err != nil {
		return false
	}
...
func (a *activeResources) localCacheCount(index stores.SectorIndex, sector abi.SectorID, worker *workerHandle) uint64 {
	...
	rpcCtx, cancel := context.WithTimeout(context.TODO(), WorkerRpcTimeout)
	out, err := worker.workerRpc.Paths(rpcCtx)
	defer cancel()
	if err != nil {
		log.Error(err)
		return cacheCount
...
func (wh *workerHandle) utilizationMine(taskType sealtasks.TaskType, a, b *workerHandle) bool {
	...
	if taskType == sealtasks.TTFinalize {
		...
		//infoA, err := a.workerRpc.Info(context.TODO())
		rpcCtx1, cancel1 := context.WithTimeout(context.TODO(), WorkerRpcTimeout)
		infoA, err := a.workerRpc.Info(rpcCtx1)
		defer cancel1()
		if err != nil {
			better = betterDefault
		}
		//infoB, err := b.workerRpc.Info(context.TODO())
		rpcCtx2, cancel2 := context.WithTimeout(context.TODO(), WorkerRpcTimeout)
		infoB, err := b.workerRpc.Info(rpcCtx2)
		defer cancel2()
		if err != nil {
			better = betterDefault
		}
		if infoA.Resources.AutoPledgeDiff == 0 || infoB.Resources.AutoPledgeDiff == 0 {
			better = betterDefault
		} else {
			workerAUsed := a.preparing.apUsed + a.active.apUsed + a.preparing.preCommit1Used + a.active.preCommit1Used
			workerBUsed := b.preparing.apUsed + b.active.apUsed + b.preparing.preCommit1Used + b.active.preCommit1Used
			workerAMax := infoA.Resources.AddPieceMax + infoA.Resources.PreCommit1Max
			workerBMax := infoB.Resources.AddPieceMax + infoB.Resources.PreCommit1Max
			if workerAMax == 0 || workerBMax == 0 {
				better = betterDefault
			} else {
				workerARate := float64(workerAUsed) / float64(workerAMax)
				workerBRate := float64(workerBUsed) / float64(workerBMax)
				better = workerARate < workerBRate
			}
		}
	}
